...
Faster AI analysis and price updating make shorter windows more workable, all else equal. When price discovery fits inside a window short enough to keep unrelated movement immaterial, conditional markets offer comparably simple exposure without fragmenting liquidity across another instrument. If the market must remain open longer, only an impact future isolates exposure to the event spread. It still cannot trade after the decision makes one outcome unrealised, so it cannot provide proposal-specific exposure beyond the decision cutoff.  

...
At a high level, this setup achieves these benefits by forcing all potential confounds through a single, narrow bottleneck: EV(metric|action is taken) / EV(metric|action is not taken). The notation EV(A|B) denotes expected value of A, conditional on B occurring. We can also represent EV(metric|action is taken) / EV(metric|action is not taken) as simply \frac{\textsf{Long}^{\text{yes}}}{\textsf{Long}^{\text{no}}}, because  The importance of EV(metric|action is taken) stems from it being the sole means by which a confounding factor can influence action selection.  

As a result, this setup only differs slightly from a scenario where action selection is entirely random and thus impossible to influence. The key difference is that confounds can now affect decisions, but only to the extent that they impact EV(metric|action is taken). This is because actions are decided according to the prices of each action's $\textsf{Long}^{\text{yes}}$ and $\textsf{Long}^{\text{no}}$ shares, which should reflect EV(metric|action is taken) and EV(metric|action is not taken).  



action is taken  
↑  
EV(metric|action is taken) / EV(metric|action is not taken)  
↑  
...
<2x causality diagrams here> (one for reverse causality and one for third variable confounding)  
